...
if __name__ == '__main__':
    jpgs_path = "P_IRRG/JPEGImages"
    pngs_path = "P_IRRG/SegmentationClass"
    classes = ["Impervious surfaces", "Building", "Low vegetation", "Tree", "Car", "_background_"]
    # classes     = ["_background_","cat","dog"]

    # 使用绝对路径
    base_dir = os.path.abspath(os.curdir)  # 获取当前目录的绝对路径
    before_dir = os.path.join(base_dir, "P_IRRG")

    if not os.path.exists(before_dir):
        print(f"Directory not found: {before_dir}")
    else:
        count = os.listdir(before_dir)
        for i in range(0, len(count)):
            path = os.path.join(before_dir, count[i])
