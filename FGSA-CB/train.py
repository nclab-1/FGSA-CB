...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = 'model_data/ep050-loss1.042-val_loss1.390.pth' #ep050-loss1.042-val_loss1.390//////////////ep040-loss0.984-val_loss1.464/ep018-loss0.980-val_loss1.418+CCLOSS/ep050-loss2.385-val_loss1.182=50.19没加领域泛化  /ep050-loss2.385-val_loss1.182+st+t =51.37/ep030-loss2.461-val_loss1.067=50.89加领域泛化
    # ---------------------------------------------------------#
    ...
    # ------------------------------------------------------------------#
    VOCdevkit_path ='VOCdevkit'    # 'VOCdevkit'  'P_IRRG'
    V_IRGB_path ='V_IRGB'        # 'V_IRGB'
    # ------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    if local_rank == 0:
        ...
        if total_step <= wanted_step:
            ...
            print(
                ...
                num_s_train, Unfreeze_batch_size, UnFreeze_Epoch, total_step))
    ...
    if True:
        ...
        t_train_dataset = DeeplabDataset(t_train_lines, P_input_shape, num_classes, True, VOCdevkit_path)
        val_dataset = DeeplabDataset(val_lines, P_input_shape, num_classes, False, VOCdevkit_path)
        ...
        s_gen = DataLoader(s_train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                           pin_memory=True,
                           drop_last=True, collate_fn=deeplab_dataset_collate, sampler=s_train_sampler)
        t_gen = DataLoader(t_train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                           pin_memory=True,
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch,
                          epoch_step, epoch_step_val, s_gen, t_gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss,
