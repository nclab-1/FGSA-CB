...
#     segfilepath = os.path.join(VOCdevkit_path, 'VOC2007/JPEGImages')  # 'VOC2007/JSegmentationClass')
...
#     classes_nums = np.zeros([256], dtype=int)  # 使用 dtype=int 替代 np.int
#     for i in tqdm(list):
#         name = total_seg[i]
...
#
#         png = np.array(Image.open(png_file_name), dtype=np.uint8)
#         if len(png.shape) > 2:
#             print(f"标签图片{name}的shape为{png.shape}，不属于灰度图或者八位彩图，请仔细检查数据集格式。")
...
#   默认指向根目录下的VOC数据集
#-------------------------------------------------------#
VOCdevkit_path      = 'V_IRGB'

if __name__ == "__main__":
    random.seed(0)
    print("Generate txt in ImageSets.")
    segfilepath     = os.path.join(VOCdevkit_path, 'VOC2007/SegmentationClass')        #'VOC2007/JSegmentationClass')
    saveBasePath    = os.path.join(VOCdevkit_path, 'VOC2007/ImageSets/Segmentation')
